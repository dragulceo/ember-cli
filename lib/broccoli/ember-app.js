...
EmberApp.prototype.removeVendorDirIfPresent = function(str) {
  return str.replace(/^vendor\//, '');
}
...
EmberApp.prototype.import = function(asset, modules) {
  ...
  var assetTree = pickFiles(directory, {
    srcDir: '/',
    destDir: this.removeVendorDirIfPresent(directory)
  });
  ...
  if (isType(assetPath, 'js')) {
    ...
    var otherAssetTree = pickFiles(this.trees.vendor, {
      srcDir: this.removeVendorDirIfPresent(directory),
      files: [basename],
      destDir: directory
    });
